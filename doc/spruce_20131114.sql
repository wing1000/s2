...
-- ----------------------------
-- Table structure for photo_search
-- ----------------------------
DROP TABLE IF EXISTS `photo_search`;
CREATE TABLE `photo_search` (
  `id_search` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `keyword` varchar(32) NOT NULL DEFAULT '',
  PRIMARY KEY (`id_search`),
  UNIQUE KEY `tag_idx` (`keyword`) USING HASH
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- ----------------------------
-- Table structure for photo_search_relation
-- ----------------------------
DROP TABLE IF EXISTS `photo_search_relation`;
CREATE TABLE `photo_search_relation` (
  `id_tag` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `id_target` bigint(20) NOT NULL,
  PRIMARY KEY (`id_tag`,`id_target`),
  UNIQUE KEY `tag_idx` (`id_target`) USING HASH
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `photo_tag` (
  ...
  PRIMARY KEY (`id_tag`),
  KEY `tag_idx` (`name`,`id_photo`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- ----------------------------
-- Table structure for photo_tag_dict
-- ----------------------------
DROP TABLE IF EXISTS `photo_tag_dict`;
CREATE TABLE `photo_tag_dict` (
  `name` varchar(32) NOT NULL DEFAULT '',
...
CREATE TABLE `user_stats` (
  ...
  `view` int(11) unsigned DEFAULT '0' COMMENT '查看次数',
  `vote` int(11) unsigned DEFAULT '0' COMMENT '投票数',
  `favorite` int(11) unsigned DEFAULT '0' COMMENT '收藏数',
  `comment` int(11) unsigned DEFAULT '0' COMMENT '评论数',
  `affection` int(11) unsigned DEFAULT '0' COMMENT '影响力，及为登录用户的vote数',
  `message` int(11) DEFAULT '0',
  `forward_message` int(11) DEFAULT '0' COMMENT '已发消息数',
  `backward_message` int(11) DEFAULT '0' COMMENT '接受的消息数',
  `update_at` bigint(20) unsigned DEFAULT '0' COMMENT '最后更新时间',
  PRIMARY KEY (`id_user`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
