...
(function($) {
	...
	$.fn.extend({
		notify : function(options) {
			...
			var parent = $('<div></div>').addClass('notice-outer').appendTo(this[0]);
			this.item = $('<div></div>').html(options.text).addClass('notice-inner').addClass(options.addClass).appendTo(parent);

			/* Sticky or auto-remove? */
			if (options.sticky) {
				var close = $('<span>x</span>').click(function() {
					self.disappear();
				}).appendTo(this.item);
			} else {
				setTimeout(function() {
