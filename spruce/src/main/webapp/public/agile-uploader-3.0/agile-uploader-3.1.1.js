(function($) {
	...
	$.fn.agileUploaderEvent = function(event) {
		...
		switch(event.type) {
			...
			case 'submitted':
				...
				break;
			case 'submit_progressing':
				$.fn.agileUploaderSubmitProgressing(event);
				break;
		}
	}
	$.fn.agileUploaderSubmitProgressing = function(event) {
		var ev = event.request;
		var bytesLoaded = ev.bytesLoaded;
		var bytesTotal = ev.bytesTotal;
		var progress = parseInt(bytesLoaded / bytesTotal * 100, 10);
		$('#progress .bar').css('width', progress + '%');

		var p = Math.floor(10000 * bytesLoaded / bytesTotal) / 100 + "%";
		console.log(p + "  " + event.request);
	}
	/**
	...
	$.fn.agileUploaderPreview = function(file) {
		if (( typeof (file.base64Thumbnail) != 'undefined') && (noURI !== true) && (file.base64Thumbnail != null)) {
			$("#preview").html('<img src="' + file.base64Thumbnail + '" />');
			$('#id-' + file.uid + ' .agileUploaderFilePreview').html('<img src="' + file.base64Thumbnail + '" />');
		} else {
			$("#preview").html('<img src="' + opts.genericFileIcon + '" />');
	...
	$.fn.agileUploaderMaxPostSize = function(file) {
		$('#id-' + file.uid).remove();
		// in case the row was visually added because it had a progress bar (it's already removed in Flash, well, it was never added actually)
		$("#agileUploaderMessages").show();
	...
	$.fn.agileUploaderFileLimit = function(file) {
		$('#id-' + file.uid).remove();
		// in case the row was visually added because it had a progress bar
		$("#agileUploaderMessages").show();
